  ...
  class TransportEvent
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MaerskClient::TransportEvent` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MaerskClient::TransportEvent`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      transport_event_type_code_validator = EnumAttributeValidator.new('String', ["ARRI", "DEPA"])
      return false unless transport_event_type_code_validator.valid?(@transport_event_type_code)
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] transport_event_type_code Object to be assigned
    def transport_event_type_code=(transport_event_type_code)
      validator = EnumAttributeValidator.new('String', ["ARRI", "DEPA"])
      unless validator.valid?(transport_event_type_code)
        fail ArgumentError, "invalid value for \"transport_event_type_code\", must be one of #{validator.allowable_values}."
      end
      @transport_event_type_code = transport_event_type_code
    end
    ...
    def hash
      [event_id, event_type, event_date_time, event_created_date_time, event_classifier_code, references, transport_event_type_code, delay_reason_code, change_remark, document_references, transport_call].hash
    end
