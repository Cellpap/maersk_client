  ...
  class Event
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MaerskClient::Event` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MaerskClient::Event`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      event_type_validator = EnumAttributeValidator.new('String', ["EQUIPMENT", "SHIPMENT", "TRANSPORT"])
    ...
    def event_type=(event_type)
      validator = EnumAttributeValidator.new('String', ["EQUIPMENT", "SHIPMENT", "TRANSPORT"])
    ...
    def hash
      [event_id, event_type, event_date_time, event_created_date_time, event_classifier_code, references].hash
    end
